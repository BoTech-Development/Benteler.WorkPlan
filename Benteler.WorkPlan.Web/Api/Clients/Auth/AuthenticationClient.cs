...
namespace Benteler.WorkPlan.Web.Api.Clients.Auth
{
    public class AuthenticationClient : ApiClientBase
    {
        ...
        /// <summary>
        /// Retrieves the current authorization information for the account asynchronously.
        /// </summary>
        /// <returns>A task that represents the asynchronous operation. The task result contains an <see
        /// cref="AuthorizationInfo"/> object with the account's authorization details, or <see langword="null"/> if no
        /// authorization information is available.</returns>
        /// <exception cref="Exception">Thrown when the authorization information cannot be retrieved due to a failed request.</exception>
        public async Task<AuthorizationInfo?> GetAuthorizationInfo() 
        {
            RequestResult<AuthorizationInfo?> result = await _httpRequestHelper.HttpGetJsonObject<AuthorizationInfo?>($"/auth/Account/GetAuthorizationInfo");
            if(result.IsSuccess())
            {
                return result.ParsedData;
            }
            ...
            {
                throw new Exception($"Could not get authorization info. \n {result.Error?.Message}");
            }
        }
        /// <summary>
